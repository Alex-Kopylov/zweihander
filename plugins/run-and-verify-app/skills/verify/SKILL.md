...
**Check Codex project skills first — even if you already know how to
...
Project skills are loaded from paths listed in `.codex/config.toml`
under `skills.config[]`. They usually live under `.codex/skills/`;
check configured paths and nearby `.codex/skills` folders.
...
- **`verifier-*` matching your surface** (CLI verifier for a CLI
  change, etc.) → load it as a Codex skill and follow its
