...
`.codex/skills/run-<unit-name>/driver.mjs` under xvfb" for a desktop
...
- Replace <unit-name> in both `name:` and `description:`. The `name:`
  becomes the project skill name (`run-<unit-name>`) and must match the
  directory name.
- The `description:` is what Codex scans to decide whether to load this
