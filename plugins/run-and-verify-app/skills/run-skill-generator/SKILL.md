...
description: Author or improve a per-project run skill that tells agents how to build, launch, and drive this project's app. Use when the user asks to set up the project, get it running, write run instructions, or verify build/run steps work from a clean environment.
---

Your job is to produce a **skill** at `<unit>/.codex/skills/run-<unit-name>/`
...
```
<unit>/.codex/skills/run-<unit-name>/
...
```
<unit>/.codex/skills/run-<unit>/
  SKILL.md         ← SHORT. Points at the driver. Has the frontmatter
                     that lets Codex load it when someone asks
...
([references/run-examples/playwright.md](../../references/run-examples/playwright.md))
— and the skill is the script that runs it. For a desktop app
([references/run-examples/electron.md](../../references/run-examples/electron.md)),
the driver is a custom REPL under tmux that exposes
`launch`/`ss`/`click`/`eval`. For a server, the driver is `curl`.
Whatever shape it takes, without something that
reaches into the running app, the skill is a description of a window
nobody can touch.

## Where the skill goes

The skill lives at `<unit>/.codex/skills/run-<unit-name>/`, where
`<unit>` is the directory for **one deployable thing** — an app, a
service, a library.

Codex loads project skills from paths listed in `.codex/config.toml`.
After creating the skill directory, add a config entry:

```toml
[[skills.config]]
path = ".codex/skills/run-<unit-name>"
enabled = true
```

Use the actual path relative to the config file. A future Codex session
can then load the skill when the request matches its description (e.g.
"run the desktop app," "take a screenshot of billing").

- **Single-project repo:** `.codex/skills/run-<repo-name>/` at repo root.
- **Large repo with many apps:** one per app, colocated —
  `apps/billing/.codex/skills/run-billing/`,
  `apps/desktop/.codex/skills/run-desktop/`.
...
the frontmatter `name:` should match — that's the project skill name.
...
(Also check for a legacy `.codex/run.md` — earlier versions of this
...
- **Build a harness to drive it.** You need a handle on the running
  ...
  **Cover the layer(s) PRs actually touch.** A tmux driver that pokes
  ...
  you don't write it (see [references/run-examples/playwright.md](../../references/run-examples/playwright.md)).
...
**The frontmatter matters.** The `name:` becomes the project skill name
(`run-billing`). The `description:` is what Codex uses to decide
whether to load this skill — put the **verbs an agent would
...
`.codex/skills/run-<unit-name>/driver.mjs` — it's long, but explicit.
...
|---|---|---|
| Web server / API | Background-launch + `curl`-based smoke script | [references/run-examples/server.md](../../references/run-examples/server.md) |
| CLI tool | Representative-args smoke script, check exit codes + output | [references/run-examples/cli.md](../../references/run-examples/cli.md) |
...
app, start from [references/run-examples/electron.md](../../references/run-examples/electron.md)
