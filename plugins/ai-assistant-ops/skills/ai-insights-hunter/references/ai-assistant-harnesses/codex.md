...
- File search and reads: use `rg`, `rg --files`, `sed`, or `nl` through `exec_command`.
- File edits: use `apply_patch` for manual edits, and keep edits scoped to the files the user confirmed.
