...
5. **Parallelize broad bumps** — if multiple independent version-bearing files must be bumped, delegate to parallel background agents using a smaller model. Give each agent only its assigned files and relevant reference files; do not mix unrelated file-type context between prompts.
