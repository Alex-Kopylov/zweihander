...
- Drop your master Typst CV (`.typ`) in the root.
...
- Edit the Typst source in `jobs/<company>/<name>_<role>_CV.typ`.
...
- The `job-hunt-toolkit:export-pdf` skill compiles the Typst source to PDF via `typst compile`.
- PDF written next to the `.typ` source with the same stem.
...
- **Don't edit PDFs.** Edit the Typst source, regenerate.
