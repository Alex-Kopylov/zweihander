...
- Tailored CV Typst source and PDF for the role; if missing or stale, invoke `job-hunt-toolkit:resume-tailoring` first
- Master CV only as evidence; never edit it for a cover letter

If the company folder or tailored CV is missing, stop and ask whether to create or run those prerequisites. Do not produce only a PDF or skip the Typst source / pre-send checks under deadline pressure.
...
- Deadlines do not waive evidence checks, Typst/PDF parity, export, or pre-send checks.
...
| Mistake | Correction |
|---|---|
| Producing only PDF | Typst is source; PDF is export |
