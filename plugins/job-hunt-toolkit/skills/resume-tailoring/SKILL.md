...
**Edit-guard:** If this skill needs to modify the master Typst source at `${JOB_HUNT_WORKSPACE:-$HOME/Documents/job_seeking}/<First>_<Last>_<Role>_CV.typ`, ask the user for explicit confirmation before doing so.
...
2. **PDF:** Invoke the `job-hunt-toolkit:export-pdf` skill for the generated Typst source. Always use absolute paths.
...
| PDF export failure | The Typst source is still saved; report error with exit code; user can use the `job-hunt-toolkit:export-pdf` skill separately |
...
- [ ] PDF was generated from that Typst source by the `job-hunt-toolkit:export-pdf` script; never create or edit PDF content directly
