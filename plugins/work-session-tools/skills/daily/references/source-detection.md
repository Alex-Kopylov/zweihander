...
| **slack** | MCP server named `slack` in `.mcp.json`, runtime settings files, or the AI agent's settings files | Check MCP config files |
...
2. Check MCP configs and runtime settings when needed for communication and custom sources
