...
| **slack** | MCP server named `slack` in `.mcp.json`, runtime settings files, or active-harness settings files | Check MCP config files; load the matching harness reference first if settings paths are harness-specific |
...
2. Check MCP configs, and active-harness settings when needed, for communication and custom sources
