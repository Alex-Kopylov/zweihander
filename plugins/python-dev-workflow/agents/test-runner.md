---
name: test-runner
description: Use this agent when you need to execute tests using pytest or uv run pytest. Supports coverage reporting when requested. Examples:\n\n<example>\nContext: User has just written a new function and wants to verify it works.\nuser: "I've added a new validation function in utils/validators.py. Can you test it?"\nassistant: "I'll use the test-runner agent to run the tests for that file."\n<Task tool call to test-runner agent with file parameter>\n</example>\n\n<example>\nContext: User completed a code change and needs to verify all tests pass.\nuser: "I've finished refactoring the payment processing logic"\nassistant: "Let me use the test-runner agent to verify all tests still pass after your refactoring."\n<Task tool call to test-runner agent without specific parameters to run all tests>\n</example>\n\n<example>\nContext: User mentions a specific test is failing.\nuser: "The 'should handle invalid card numbers' test is failing"\nassistant: "I'll use the test-runner agent to run that specific test and show you the detailed error."\n<Task tool call to test-runner agent with specific test name>\n</example>\n\n<example>\nContext: User wants to check test coverage.\nuser: "Run tests with coverage"\nassistant: "I'll use the test-runner agent to run all tests with coverage reporting."\n<Task tool call to test-runner agent with coverage enabled>\n</example>\n\n<example>\nContext: User wants to run only unit tests.\nuser: "Run unit tests"\nassistant: "I'll use the test-runner agent to run unit tests."\n<Task tool call to test-runner agent for unit tests>\n</example>\n\n<example>\nContext: User wants to run only integration tests.\nuser: "Run integration tests"\nassistant: "I'll use the test-runner agent to run integration tests."\n<Task tool call to test-runner agent for integration tests>\n</example>\n\n<example>\nContext: Proactive usage after code changes based on CLAUDE.md requirements.\nuser: "Please add error handling to the createTransaction function"\nassistant: "Here's the updated function with error handling:"\n<code changes>\nassistant: "Now I'll use the test-runner agent to verify all tests still pass."\n<Task tool call to test-runner agent>\n</example>
model: sonnet
color: purple
skills:
  - pytest-redis
