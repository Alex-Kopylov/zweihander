...
allowed-tools: AskUserQuestion, Agent, Skill(python-dev-workflow:commit, general-plugins:pr-comment)
---
...
4. **Reply to the thread** — invoke `Skill(general-plugins:pr-comment)` to post a reply explaining what was done (e.g. `"Fixed: extracted validation into helper"`)
...
- **Commit changes** — invoke `Skill(python-dev-workflow:commit)`
