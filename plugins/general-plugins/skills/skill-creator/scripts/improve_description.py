...
def _call_assistant(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    """
    configured_command = os.environ.get("SKILL_CREATOR_ASSISTANT_COMMAND")
    if not configured_command:
        raise RuntimeError("SKILL_CREATOR_ASSISTANT_COMMAND is required")
    cmd = shlex.split(configured_command)
    if model:
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = _call_assistant(prompt, model)
    ...
    # This subprocess path is one-shot, so we
    ...
    if len(description) > 1024:
        ...
        shorten_text = _call_assistant(shorten_prompt, model)
