...
that convention. If the repository keeps Claude Code adapters next to shared
AGENTS.md files, check sibling `CLAUDE.md` files for `@AGENTS.md` imports and
treat them as harness adapters unless the user asks to audit them directly.
...
After outputting the quality report, ask the user for confirmation with Skill(AskUserQuestion) before updating.
