...
The last gate before a PDF goes out. This skill drives the checklist section-by-section, delegates metadata scrubbing to Skill(job-hunt-toolkit:scrub-pdf-metadata), collects results, and refuses to declare "ready" unless every gate passes.
...
if [[ "$(stat -f%m "$html")" -gt "$(stat -f%m "$pdf")" ]]; then
  echo 'FAIL: HTML edited after PDF exported. Re-run Skill(job-hunt-toolkit:export-pdf).'
  exit 1
fi
```
...
**Invoke Skill(job-hunt-toolkit:scrub-pdf-metadata)** on the PDF; do not inline exiftool commands here.
...
- **Always run Skill(job-hunt-toolkit:scrub-pdf-metadata) in Section 3.** Never skip even if the user says they "just scrubbed" — HTML edits invalidate prior scrubs.
- **Cross-company leak = catastrophic.** Any other company name in the PDF text or HTML comments is game-over; block.
- **Defer metadata specifics to Skill(job-hunt-toolkit:scrub-pdf-metadata) and its references.** If you need to fall back, use EXACT commands from `exiftool-commands.md` — never improvise.
