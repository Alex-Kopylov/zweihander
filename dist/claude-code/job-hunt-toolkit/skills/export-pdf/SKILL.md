...
- Invoked manually by the user after editing HTML. Also called by Skill(job-hunt-toolkit:prepare-to-send) to verify PDF freshness. NOT called by Skill(job-hunt-toolkit:new-application) (user tailors HTML first, then exports the PDF).
...
- **HTML file** (argument or inferred): path to the source HTML. If omitted:
  ...
  - If multiple, ask the user with Skill(AskUserQuestion) which one.
...
Invoke Skill(job-hunt-toolkit:scrub-pdf-metadata) on the produced PDF as the final step.

Every exported PDF is scrubbed, even when attached directly without Skill(job-hunt-toolkit:prepare-to-send).
...
- **Always scrub metadata after export.** Invoke Skill(job-hunt-toolkit:scrub-pdf-metadata); Skill(job-hunt-toolkit:prepare-to-send) also verifies scrubbing.
...
3. Run Skill(job-hunt-toolkit:prepare-to-send) before attaching to any application for a final freshness and content check
