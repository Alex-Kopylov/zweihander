...
3. Ask a targeted question only when a critical gap cannot be handled truthfully by omission or disclosure — use Skill(AskUserQuestion) for bounded factual blockers and plain chat for open-ended discovery; use patterns from `references/branching-questions.md`:
