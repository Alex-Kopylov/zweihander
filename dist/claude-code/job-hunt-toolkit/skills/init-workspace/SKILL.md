...
- **HTML is the source, PDF is the export.** Never hand-edit a PDF. Edit the HTML, then regenerate the PDF with Skill(job-hunt-toolkit:export-pdf).
- **Master HTML is edit-guarded.** The master HTML at the workspace root is canonical and irreplaceable. Ask the user for explicit confirmation with Skill(AskUserQuestion) before you overwrite or modify it. The master PDF is a build artifact and may be overwritten freely. Tailored variants live in `<workspace-root>/<company>/`.
...
Next:
  ...
  - Use Skill(job-hunt-toolkit:export-pdf) to generate the master PDF from HTML
```
