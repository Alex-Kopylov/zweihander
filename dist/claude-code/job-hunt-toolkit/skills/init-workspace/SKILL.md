...
- **Workspace path** (optional argument): where to create the workspace. Default: the `JOB_HUNT_WORKSPACE` env var, else `~/Documents/job_seeking`. Resolve `~` to the real home.
...
- If the directory already exists AND contains `AGENTS.md`, ask the user with Skill(AskUserQuestion):
...
- **Typst is the source, PDF is the export.** Never hand-edit a PDF. Edit the `.typ` source, then regenerate the PDF with Skill(job-hunt-toolkit:export-pdf).
- **The master Typst source is edit-guarded.** The master `.typ` at the workspace root is canonical and irreplaceable. Ask the user for explicit confirmation with Skill(AskUserQuestion) before you overwrite or modify it. The master PDF is a build artifact and may be overwritten freely. Tailored variants live in `<workspace-root>/jobs/<company>/`.
...
Next:
  - Use Skill(job-hunt-toolkit:new-application) with <company-slug> to start an application
  - Use Skill(job-hunt-toolkit:export-pdf) to generate the master PDF from the Typst source
```

## References

Shared across every skill in this plugin — read these when a skill asks you to:

- [naming-rules.md](../../references/naming-rules.md) — file and folder naming
- [workspace-layout.md](../../references/workspace-layout.md) — directory structure
- [application-lifecycle.md](../../references/application-lifecycle.md) — end-to-end flow
