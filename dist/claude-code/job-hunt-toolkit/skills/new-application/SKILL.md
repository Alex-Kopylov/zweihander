...
If the workspace doesn't exist or lacks a master CV, stop and direct the user to use Skill(job-hunt-toolkit:init-workspace) first.
...
Next:
  ...
  - Use Skill(job-hunt-toolkit:resume-tailoring) to tailor the CV
  - Use Skill(job-hunt-toolkit:export-pdf) to regenerate the PDF
  - Use Skill(job-hunt-toolkit:prepare-to-send) before sending
```
