...
`$ARGUMENTS` holds the slash-command version argument.
...
4. **Parallelize broad bumps** — if multiple independent version-bearing files must be bumped, delegate with Skill(Agent) to parallel background agents using a smaller model, such as `haiku`. Give each agent only its assigned files and relevant reference files; do not mix unrelated file-type context between prompts.
...
7. **Report completion** — after verification succeeds, show the old and new versions, bump reason, updated files, and verification result.
8. **Commit when requested** — stage only changed version files and invoke Skill(dev-workflow:commit) with type `bump` and description `vOLD → vNEW`.
