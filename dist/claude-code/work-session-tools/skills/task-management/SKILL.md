...
> **Note:** TaskCreate, TaskGet, TaskList, and TaskUpdate use `taskId` (camelCase). TaskOutput and TaskStop use `task_id` (snake_case).
...
Use `Explore`, `Plan`, `general-purpose`, and named subagents only when the
active environment exposes them.
