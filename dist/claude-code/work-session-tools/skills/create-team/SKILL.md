...
6. **Model tier** - ask with Skill(AskUserQuestion) what capability level each role should use
...
Track progress with Skill(TaskCreate) and Skill(TaskUpdate), keeping one item in progress. Spawn parallel agents with Skill(Agent) only when supported and justified by the team design.
