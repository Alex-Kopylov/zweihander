...
Invoke Skill(work-session-tools:daily) with optional `output-dir`.
...
When source detection or channel discovery needs assistant-specific files,
include Claude Code project instructions such as `CLAUDE.md` and Claude Code
settings or MCP configuration exposed in the active environment. Use Claude
Code MCP tools available in the active session for Slack, Microsoft Teams,
Telegram, Azure DevOps, and custom MCP sources.

Fetch items **created, updated, or deleted** within the time window. If a source fails or is unavailable — including a configured server not exposed as an available tool — note it and move on; never block the entire note on one source.
