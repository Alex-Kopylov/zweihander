...
You can omit `expectedOutput`; experiments still produce traces that Langfuse LLM-as-Judge evaluators can score. If the user asks to configure those evaluators next, suggest or delegate to the `langfuse:langfuse-eval-manager` agent with Skill(Agent).
