...
3. **Input schema** — whether to enforce JSON Schema on item inputs; if yes, invoke Skill(langfuse:design-dataset-schema) to help design it.
...
3. Suggest next steps:
   - "Delegate to the `langfuse:langfuse-dataset-expert` agent with Skill(Agent) to add items."
   - "Invoke Skill(langfuse:design-dataset-schema) if you want to add schemas later."
