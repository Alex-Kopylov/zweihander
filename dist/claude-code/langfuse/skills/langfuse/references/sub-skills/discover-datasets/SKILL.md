...
- If dataset has no items → "Delegate to the `langfuse:langfuse-dataset-expert` agent with Skill(Agent) to populate it."
- If dataset has items but no runs → "Delegate to the `langfuse:langfuse-experiment-manager` agent with Skill(Agent) to run experiments."
- If dataset has no schema → "Consider adding input/output schemas for validation."
- If remote experiment URL is not configured → "Delegate to the `langfuse:langfuse-experiment-manager` agent with Skill(Agent) to configure remote triggering."
