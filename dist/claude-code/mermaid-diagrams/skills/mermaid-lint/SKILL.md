...
2. If `mmdc` is missing, stop and ask the user with Skill(AskUserQuestion) whether to install it:

   ```text
   Ask: "mmdc is missing. Install with npm install -g @mermaid-js/mermaid-cli@latest before linting? (yes/no)"
   If yes: npm install -g @mermaid-js/mermaid-cli@latest
   ```

3. If `CURR_VERSION` is not empty and differs from `LATEST_VERSION`, pause the process and ask the user with Skill(AskUserQuestion) whether to update or continue without it:

   ```text
   Ask: "mmdc is outdated ($CURR_VERSION -> $LATEST_VERSION). Update now, or proceed without it? (update/continue)"
   ```
