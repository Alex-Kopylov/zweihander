...
2. If `mmdc` is missing, stop and ask the user with $request_user_input whether to install it:
...
3. If `CURR_VERSION` is not empty and differs from `LATEST_VERSION`, pause the process and ask the user with $request_user_input whether to update or continue without it:
