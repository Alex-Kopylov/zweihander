...
- If dataset has no items → "Delegate to the `langfuse:langfuse-dataset-expert` agent with $spawn_agent to populate it."
- If dataset has items but no runs → "Delegate to the `langfuse:langfuse-experiment-manager` agent with $spawn_agent to run experiments."
- If dataset has no schema → "Consider adding input/output schemas for validation."
- If remote experiment URL is not configured → "Delegate to the `langfuse:langfuse-experiment-manager` agent with $spawn_agent to configure remote triggering."
