...
3. **Input schema** — whether to enforce JSON Schema on item inputs; if yes, invoke $langfuse:design-dataset-schema to help design it.
...
3. Suggest next steps:
   - "Delegate to the `langfuse:langfuse-dataset-expert` agent with $spawn_agent to add items."
   - "Invoke $langfuse:design-dataset-schema if you want to add schemas later."
