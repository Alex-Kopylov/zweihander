...
- Invoked manually by the user after editing HTML. Also called by $job-hunt-toolkit:prepare-to-send to verify PDF freshness. NOT called by $job-hunt-toolkit:new-application (user tailors HTML first, then exports the PDF).
...
- **HTML file** (argument or inferred): path to the source HTML. If omitted:
  ...
  - If multiple, ask the user with $request_user_input which one.
...
Invoke $job-hunt-toolkit:scrub-pdf-metadata on the produced PDF as the final step.

Every exported PDF is scrubbed, even when attached directly without $job-hunt-toolkit:prepare-to-send.
...
- **Always scrub metadata after export.** Invoke $job-hunt-toolkit:scrub-pdf-metadata; $job-hunt-toolkit:prepare-to-send also verifies scrubbing.
