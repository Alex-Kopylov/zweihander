...
3. Ask a targeted question only when a critical gap cannot be handled truthfully by omission or disclosure — use $request_user_input for bounded factual blockers and plain chat for open-ended discovery; use patterns from `references/branching-questions.md`:
...
**Edit-guard:** If this skill needs to modify the master Typst source at `${JOB_HUNT_WORKSPACE:-$HOME/Documents/job_seeking}/<First>_<Last>_<Role>_CV.typ`, ask the user for explicit confirmation with $request_user_input before doing so.
...
2. **PDF:** Invoke $job-hunt-toolkit:export-pdf for the generated Typst source. Always use absolute paths.
...
| PDF export failure | The Typst source is still saved; report error with exit code; user can use $job-hunt-toolkit:export-pdf separately |
...
- [ ] PDF was generated from that Typst source by $job-hunt-toolkit:export-pdf; never create or edit PDF content directly
