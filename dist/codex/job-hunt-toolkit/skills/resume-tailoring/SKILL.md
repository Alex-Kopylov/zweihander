...
**Edit-guard:** If this skill needs to modify the master HTML at `${JOB_HUNT_WORKSPACE:-$HOME/Documents/job_seeking}/<First>_<Last>_<Role>_CV.html`, ask the user for explicit confirmation with $request_user_input before doing so.
...
2. **PDF:** Invoke $job-hunt-toolkit:export-pdf for the generated HTML. Always use absolute paths.
...
| PDF export failure | HTML is still saved; report error with exit code; user can use $job-hunt-toolkit:export-pdf separately |
...
- [ ] PDF was generated from that HTML by $job-hunt-toolkit:export-pdf; never create or edit PDF content directly
