...
If the workspace doesn't exist or lacks a master CV, stop and direct the user to use $job-hunt-toolkit:init-workspace first.
...
- Check `<workspace>/jobs/<slug>/` doesn't already exist. If it does, ask with $request_user_input:
...
Ask the user with $request_user_input how they'll provide the JD:
...
Ask the user with $request_user_input to choose:
- **Tailor now** → invoke $job-hunt-toolkit:resume-tailoring with the JD and this company's Typst source as context
...
Next:
  ...
  - Use $job-hunt-toolkit:resume-tailoring to tailor the CV
  - Use $job-hunt-toolkit:export-pdf to regenerate the PDF
  - Use $job-hunt-toolkit:prepare-to-send before sending
```
...
| Scenario | Action |
|---|---|
| Workspace doesn't exist | Stop; direct user to $job-hunt-toolkit:init-workspace |
| Master Typst source not found | Stop; ask user to create one or use $job-hunt-toolkit:init-workspace |
