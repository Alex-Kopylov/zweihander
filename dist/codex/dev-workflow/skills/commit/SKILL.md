...
Ask the user whether to push, using $request_user_input, and include the push preview. Preview = actual output of `git log @{u}..HEAD --oneline`. If no upstream, show only the new commit and note that push will require `--set-upstream`.
