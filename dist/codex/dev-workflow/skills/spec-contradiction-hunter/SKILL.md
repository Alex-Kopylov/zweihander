...
Delegate with $spawn_agent to all applicable contradiction-hunter agents in a **single message**. Pass the full collected information in each agent's prompt.
...
Review the collected material for domains where specialized knowledge would reveal contradictions the default agents may miss. For each clearly warranted domain, delegate with $spawn_agent to an additional `general-purpose` agent with a stronger model and a tailored prompt. Avoid speculative delegation.
...
3. Present the consolidated report and ask the user with $request_user_input to confirm, clarify, or resolve each contradiction
