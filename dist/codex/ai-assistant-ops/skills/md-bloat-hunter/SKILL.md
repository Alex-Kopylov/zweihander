...
- User approval questions: use $request_user_input when a structured
  question is useful; otherwise ask in chat and wait.
- Parallel dispatch: delegate the current fan-out with
  $spawn_agent in one batch when your runtime supports it,
...
has not asked to skip PR comments, invoke $dev-workflow:pr-comment
...
deletion as the offered solution so $dev-workflow:pr-comment can
