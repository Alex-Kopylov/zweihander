...
Search Codex session logs under `~/.codex/sessions/` for recent `.jsonl`
files that match the current project path or session timing. The most recent
match is the current session's main log.
...
For each conversation log, read all agent specs defined in `agents/`, then spawn one agent per hunter spec with $spawn_agent in a single message when your runtime supports parallel delegation. Pass the full log content to each agent's prompt.
...
Ask the user with $request_user_input to choose exactly one bounded option:
...
Walk through plan items one-by-one. Ask the user with $request_user_input to decide for each item: store it? where?

When a preview is needed and the input mechanism does not support preview
...
Invoke $ai-assistant-ops:agents-md-improver when available, scoped only to the files written during Step 6.
