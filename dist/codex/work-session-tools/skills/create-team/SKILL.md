...
6. **Model tier** - ask with $request_user_input what capability level each role should use
...
Track progress with $update_plan and $update_plan, keeping one item in progress. Spawn parallel agents with $spawn_agent only when supported and justified by the team design.
