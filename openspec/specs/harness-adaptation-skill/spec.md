...
The skill SHALL document that frontmatter is the portability boundary: which keys are portable, the `allowed_tools` global as the only route for an `allowed-tools` grant, both per-harness placements of that key, the merge of a hand-written `metadata:` block, and the namespaced `metadata` shape.

#### Scenario: Boundary documented
- **WHEN** the skill content is checked against the renderer's frontmatter behavior
- **THEN** the portable key set, the global's call form, both placements of `allowed-tools`, and the metadata namespaces are documented in the skill
