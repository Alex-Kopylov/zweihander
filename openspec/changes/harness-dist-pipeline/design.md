...
- `plugin_maintenance.build --check` renders both trees to a temporary directory and reports the paths that differ from the committed ones, without writing to the tree. It is a local convenience, not a second gate: the CI diff stays the authority because it also covers stage-1 output written into `plugins/`.
