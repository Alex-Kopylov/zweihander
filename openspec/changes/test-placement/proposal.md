...
- **Dist-invariant tests switch from reading committed `dist/` to reading `rendered`.** Committed `dist/` stays the CI gate's concern; the only test that still reads it is the freshness check (`stale_paths()`), which is a build-layer test.
- **Two new source-policy tests** in `tests/unit/plugin_maintenance/`: no `tests/` directory under `plugins/**`, and no file under `tests/` outside `tests/unit/plugin_maintenance/` (plus `tests/conftest.py`) contains the strings `.j2` or `plugins/`. Together they make a repeat of `8c987e1` a test failure.
